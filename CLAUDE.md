...
| 호출 | 마크 | 직함 | 영역 | 비용 | 설치 |
|------|------|------|------|------|------|
| `grok` / `gr` | **`_Grok`** | **웹 코드 디자이너 · 다큐 PD** | ① 웹페이지·웹 코드 ② 누나 사진 1장+프롬프트→10초 딥페이크+더빙→다큐 | 45,000원/월 | ✅ |
...
> - **그림·PD 로봇 (Grok, `gr`):** 플러그는 두 칸만! 웹페이지를 그리고 코드를 짜거나, 누나 사진 한 장과 프롬프트로 10초 딥페이크 클립을 만들어 더빙하고 다큐로 이어 붙여. 월 구독료 45,000원.
...
- **Grok은 웹 코드·다큐 PD 두 칸만, 반장은 패치, 출판부는 전시장 게이트, 감사는 「아니다」** — 직함 침범 최소화
...
> - **그림·PD 로봇은 웹 코드와 다큐 두 칸만, 고치기 로봇은 수리, 심판은 "안 돼!"** — 서로 자기 일만 하기.
...
- **게이트 소유는 출판부 `_Claude`.** Grok은 칸 ①에서 손댄 페이지를 빌드한다. 세션에서 수첩을 새로 쓰면:
  ```bash
