# 냉장고(Fridge) 아키텍처 — REDACTED ⇄ helena751107 자산 공유 체계
...
# 전체 28종 인벤토리
gh api /user/repos \
  --jq '[.[] | select(.owner.login == "REDACTED")] | .[] | "\(.name) | \(.description)"'
```

### 4.2 특정 자산 즉시 복제
```bash
# termux-bridge 필요하면:
gh repo clone REDACTED/termux-bridge /root/fridge/termux-bridge
...
```bash
# README만 읽기
gh api repos/REDACTED/parksy-audio/contents/README.md --jq '.content' | base64 -d
...
for r in $(gh api /user/repos --jq '[.[]|select(.owner.login=="REDACTED")].name[]'); do
  [ -d "/root/fridge/$r" ] && git -C "/root/fridge/$r" pull --ff-only \
    || gh repo clone "REDACTED/$r" "/root/fridge/$r"
done
```
...
1. **REDACTED가 만들면, helena751107 콜라보는 기본값.** 새 레포 생성 시 자동으로 초대.
2. **AI 에이전트는 양쪽 레포를 균등하게 취급.** `helena_phone/scripts/`와 `REDACTED/termux-bridge/`는 같은 냉장고의 다른 선반.
3. **자산 출처는 기록하되, 재사용을 막지 않는다.** "어디서 왔는지"는 밝히고, "써도 되는지"는 묻지 않는다.
4. **냉장고는 계속 채워진다.** REDACTED의 새 실험·템플릿·파이프가 생기면 자동으로 helena751107의 자산 풀에 추가.
...
냉장고는 **창작자(REDACTED)와 수혜자(helena751107)가
