...
def call_deepseek(prompt: str, max_tokens: int = 2000) -> str:
    ...
    data = json.dumps({
        "model": DS_MODEL,
        "messages": [
            {"role": "system", "content": "너는 대한민국 라디오 방송국에 사연을 보내는 전문 작가다. 진심이 묻어나는 편지글을 쓴다. 과장하지 않고, 진솔하게, 구체적인 디테일을 담는다. 절대 AI가 쓴 티를 내지 않는다."},
...
def generate_story(channel_type: str, performance: dict) -> dict:
    ...
    return {
        "channel": CONFIG["channels"][channel_type]["name"],
